...
namespace Lirp
{
	public class TerrainMeshMold {
	    ...
	    static Vector3 GetTerrainPos(Terrain terrain, int ix, int iz,float ht)
	    {
	        ...
	        worldPos.x += terrain.terrainData.size.x * ((float)(ix)) / ((float)(terrain.terrainData.heightmapResolution - 1));
	        worldPos.y += ht* terrain.terrainData.size.y;
	        worldPos.z += terrain.terrainData.size.z * ((float)(iz)) / ((float)(terrain.terrainData.heightmapResolution - 1));
	        return worldPos;
	    }
